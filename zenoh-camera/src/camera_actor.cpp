...
CameraActor::CameraActor(const char *name, size_t stack_size, int priority, size_t queue_depth) : Actor<CameraEvent, CameraCmd>(stack_size, name, priority, queue_depth)
{
    ...
    _camera_config = {
        ...
        .frame_size = FRAMESIZE_VGA ,// was FRAMESIZE_UXGA,   // QQVGA-UXGA, For ESP32, do not use sizes above QVGA when not JPEG. The performance of the ESP32-S series has improved a lot, but JPEG mode always gives better frame rates.
