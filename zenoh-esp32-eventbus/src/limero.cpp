 ...
 Result<Sample*> Sample::cbor_deserialize(const Bytes& bytes) {
    ...
    while (!cbor_value_at_end(&mapIt)) {
        ...
        switch (key) {
            ...
            case Sample::Field::FLAG_INDEX:{cbor_value_get_boolean(&mapIt, &(*msg->flag));cbor_value_advance(&mapIt);
        ...
        *msg->name = std::string(valbuf, vallen - 1);
            ...
            case Sample::Field::VALUES_INDEX:{CborValue tmp;
                ...
                while (!cbor_value_at_end(&tmp)) {
                    float v;
                    cbor_value_get_float(&tmp, &(v));cbor_value_advance(&tmp);
            ...
            case Sample::Field::F_INDEX:{cbor_value_get_float(&mapIt, &(*msg->f));cbor_value_advance(&mapIt);

                break;
            }
            
            case Sample::Field::D_INDEX:{cbor_value_get_double(&mapIt, &(*msg->d));cbor_value_advance(&mapIt);
...
Result<Bytes> LogInfo::cbor_serialize(const LogInfo& msg)  {
 ...
 Result<LogInfo*> LogInfo::cbor_deserialize(const Bytes& bytes) {
    ...
    *msg->line = v;
            ...
            case LogInfo::Field::TIMESTAMP_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->timestamp));cbor_value_advance(&mapIt);
 ...
 Result<SysCmd*> SysCmd::cbor_deserialize(const Bytes& bytes) {
    ...
    if (cbor_value_is_text_string(&mapIt)) {
        ...
        msg->src = std::string(valbuf, vallen - 1);
            ...
            case SysCmd::Field::SET_TIME_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->set_time));cbor_value_advance(&mapIt);

                break;
            }
            
            case SysCmd::Field::REBOOT_INDEX:{cbor_value_get_boolean(&mapIt, &(*msg->reboot));cbor_value_advance(&mapIt);
 ...
 Result<SysInfo*> SysInfo::cbor_deserialize(const Bytes& bytes) {
    ...
    while (!cbor_value_at_end(&mapIt)) {
        ...
        switch (key) {
            ...
            case SysInfo::Field::UTC_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->utc));cbor_value_advance(&mapIt);

                break;
            }
            
            case SysInfo::Field::UPTIME_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->uptime));cbor_value_advance(&mapIt);

                break;
            }
            
            case SysInfo::Field::FREE_HEAP_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->free_heap));cbor_value_advance(&mapIt);

                break;
            }
            
            case SysInfo::Field::FLASH_INDEX:{cbor_value_get_uint64(&mapIt, &(*msg->flash));cbor_value_advance(&mapIt);
