...
private:
    ...
    void process_frame() {
        if (rx_index < 6) return; // Minimum frame size
...
public:
    void serial_send(const uint8_t* data, uint16_t len) override {
        // Implement serial transmission for your platform
        // Example: write to UART, USB serial, etc.
        for (uint16_t i = 0; i < len; i++) {
            // platform_serial_write(data[i]);
        }
    }
