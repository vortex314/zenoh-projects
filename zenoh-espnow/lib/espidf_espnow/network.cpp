...
#include <WiFiUdp.h>
    ...
    z_result_t _z_create_endpoint_tcp(_z_sys_net_endpoint_t *ep, const char *s_address, const char *s_port)
    {
        ...
        if ((ep->_iptcp._port < (uint32_t)1) ||
            (ep->_iptcp._port > (uint32_t)65355))
        { // Port numbers should range from 1 to 65355
            ret = _Z_ERR_GENERIC;
        }
...
#error "Bluetooth not supported yet on OpenCR port of Zenoh-Pico"
    ...
    std::vector<uint8_t> rxd_packet;
    #define READ_TIMEOUT 20 // as small as possible but enough to catch handshake ( 10, 33, 83 ) bytes , at 921600 it's around 100 bytes/msec
    ...
    size_t _z_read_serial_internal(const _z_sys_net_socket_t sock, uint8_t *header, uint8_t *ptr, size_t len)
    {
        ...
        while (millis() < timeout && rxd_packet.size() < _Z_SERIAL_MAX_COBS_BUF_SIZE )
        {
            ...
            {
                uint8_t data = sock._serial->read();
                rxd_packet.push_back(data);
                if (data == 0x00)
                {
                    ret = _z_serial_msg_deserialize(rxd_packet.data(), rxd_packet.size(), ptr, len, header, tmp_buf, _Z_SERIAL_MFS_SIZE);
                    if (ret == SIZE_MAX)
                    {
                        rxd_packet.clear();
                    }
